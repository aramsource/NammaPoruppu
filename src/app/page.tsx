...
export default async function Home() {
  ...
  let wards: Ward[] = [];
  try {
    const [{ data: reportsData, error: repErr }, { data: imagesData, error: imgErr }, { data: wardsData, error: wardsErr }] = await Promise.all([
      supabase.from("reports").select("*").order("created_at", { ascending: false }).limit(500),
      supabase.from("report_images").select("report_id,image_url,created_at").order("created_at", { ascending: true }),
      supabase.from("wards").select("*").order("ward_number", { ascending: true }),
    ]);
    if (repErr || imgErr || wardsErr) {
      throw new Error(repErr?.message ?? imgErr?.message ?? wardsErr?.message ?? "Failed to load home data");
    }
    reports = (reportsData ?? []) as Array<Record<string, unknown>>;
    images = (imagesData ?? []) as Array<Record<string, unknown>>;
    const geoWardIndex = await getChennaiWardIndex();
    const mergedByWardNo = new Map<number, Ward>(
      geoWardIndex.map((w) => [
        w.wardNumber,
        {
          id: w.id,
    ...
    for (const w of wardsData ?? []) {
      mergedByWardNo.set(w.ward_number, {
        id: w.id,
    ...
    wards = [...mergedByWardNo.values()].sort((a, b) => a.wardNumber - b.wardNumber);
  } catch (err: unknown) {
    loadError = err instanceof Error ? err.message : "Failed to load home data";
  }
  const wardsSource: Ward[] = wards;
  ...
  const reportsSource: Report[] = (reports ?? [])
    .map((r) => {
  ...
  const wardStats = wardsSource.map((w) => {
  ...
  const catStats = issueCategories.map((cat) => {
    ...
    <main className="min-h-[calc(100vh-64px)]">
      ...
      <section className="relative overflow-hidden bg-accent-600">
        ...
        <div className="relative mx-auto max-w-5xl px-4 pt-12 pb-12 md:pt-28 md:pb-20">
          ...
          {/* Headline: "Namma" plays on the platform name */}
          ...
          <p className="mt-6 max-w-md text-[1.05rem] font-medium leading-relaxed text-white/90 md:mt-7 md:text-lg">
            Report civic problems, build community pressure, and track who&apos;s responsible for fixing them right here in Chennai.
          </p>
      ...
      <section className="border-y border-white/10 bg-slate-900">
        <div className="mx-auto max-w-5xl px-4 py-3">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <p className="text-[11px] font-bold uppercase tracking-widest text-white/55">Live civic proof</p>
            {latestDataAt ? (
        ...
        <div className="mx-auto max-w-5xl">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Most supported open reports right now.</p>
            </div>
        ...
        <div className="mx-auto max-w-5xl">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Resolution metrics across all monitored wards.</p>
            </div>
      ...
      <section className="bg-accent-600 px-4 py-16 md:py-20">
        <div className="mx-auto max-w-2xl text-center text-white">
          ...
          <p className="mt-4 text-accent-200">
            Report it in under 60 seconds and get the right official notified.
          </p>
          <Link
            href="/report-issue"
